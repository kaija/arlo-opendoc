...
- [ ] 1. Add shared search types to `packages/shared`
  - [ ] 1.1 Create `packages/shared/src/types/search.ts` with four exported interfaces
  ...
  - [ ] 1.2 Re-export all four new interfaces from `packages/shared/src/index.ts`
...
- [ ] 2. Extend `WorktreeTabState` with `scrollToLine`
  - [ ] 2.1 Add `scrollToLine: number | null` to `WorktreeTabState` and `EMPTY_TAB_STATE` in `apps/desktop/src/renderer/src/types.ts`
...
- [ ] 3. Add `ClientInterface` methods and IPC bindings in `packages/client`
  - [ ] 3.1 Declare `searchFiles` and `findInFiles` method signatures on `ClientInterface` in `packages/client/src/types.ts`
  ...
  - [ ] 3.2 Implement `searchFiles` and `findInFiles` bindings in `packages/client/src/ipc.ts`
...
- [ ] 4. Implement `RipgrepRunner` in the main process
  - [ ] 4.1 Create `apps/desktop/src/main/ripgrepRunner.ts` with binary resolution and process spawning
  ...
  - [ ] 4.2 Implement NDJSON output parser in `ripgrepRunner.ts`
  ...
  - [ ]* 4.3 Write unit tests for `RipgrepRunner` in `apps/desktop/src/main/__tests__/ripgrepRunner.test.ts`
  ...
  - [ ]* 4.4 Write property test for `RipgrepRunner` file count upper bound
  ...
  - [ ]* 4.5 Write property test for `RipgrepRunner` total match line upper bound
  ...
  - [ ]* 4.6 Write property test for `ContentMatchLine` line number monotonicity
...
- [ ] 5. Register IPC handlers in the main process
  - [ ] 5.1 Register `ipcMain.handle("arlo-doc:searchFiles", …)` in `apps/desktop/src/main/index.ts`
  ...
  - [ ] 5.2 Register `ipcMain.handle("arlo-doc:findInFiles", …)` in `apps/desktop/src/main/index.ts`
    - Validate `repoDir`, short-circuit empty query, delegate to `RipgrepRunner.findInFiles`
    - Wrap all exceptions via `wrapError`
    - _Requirements: REQ-006.4, REQ-006.6, REQ-006.7_

  - [ ]* 5.3 Write property test for IPC handler error contract
...
- [ ] 6. Checkpoint — Ensure all back-end and shared-layer tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Create `fileNameMatcher.ts` in the renderer
  - [ ] 7.1 Create `apps/desktop/src/renderer/src/fileNameMatcher.ts` with `flattenLeaves`, `scoreFileName`, and `matchFileNames`
  ...
  - [ ]* 7.2 Write unit tests for `fileNameMatcher` in `apps/desktop/src/renderer/src/__tests__/fileNameMatcher.test.ts`
    - Empty query returns `[]`
  ...
  - [ ]* 7.3 Write property test for file name search result limit
  ...
  - [ ]* 7.4 Write property test for file name search result ordering
  ...
  - [ ]* 7.5 Write property test for case-insensitivity idempotence
  ...
  - [ ]* 7.6 Write property test for regex mode results satisfy pattern
...
- [ ] 8. Checkpoint — Ensure all file-name matcher tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Replace `SearchModal` component
  - [ ] 9.1 Replace `apps/desktop/src/renderer/src/components/SearchModal.tsx` with full implementation
  ...
  - [ ] 9.2 Implement Search Files tab behaviour in `SearchModal`
  ...
  - [ ] 9.3 Implement Find in Files tab behaviour in `SearchModal`
  ...
  - [ ] 9.4 Implement search options toggles (**Aa** / **.\***) in `SearchModal`
  ...
  - [ ] 9.5 Implement keyboard navigation in `SearchModal`
  ...
  - [ ] 9.6 Implement footer count display in `SearchModal`
  ...
  - [ ]* 9.7 Write unit tests for `SearchModal` in `apps/desktop/src/renderer/src/__tests__/search.property.test.ts`
  ...
  - [ ]* 9.8 Write property test for footer count accuracy
...
- [ ] 10. Wire keyboard shortcut and `handleSearchResultClick` in `App.tsx`
  - [ ] 10.1 Add `Cmd+Shift+F` / `Ctrl+Shift+F` keyboard shortcut listener to `App.tsx`
  ...
  - [ ] 10.2 Add `getAncestorPaths(filePath: string): string[]` pure helper to `App.tsx` (exported for testing)
    - Split path on `'/'`, accumulate each prefix, return as array of ancestor path strings
    - _Requirements: REQ-009.4_

  - [ ] 10.3 Implement `handleSearchResultClick(filePath, lineNumber?)` in `App.tsx`
  ...
  - [ ] 10.4 Pass `SearchModal` props through `MainLayout` — add `onSearchResultClick` prop and render `SearchModal` when `state.modal === 'search'`
  ...
  - [ ]* 10.5 Write unit tests for `handleSearchResultClick` and keyboard shortcut in `apps/desktop/src/renderer/src/__tests__/search.property.test.ts`
  ...
  - [ ]* 10.6 Write property test for `scrollToLine` clamping
...
- [ ] 11. Update `DocumentView` to consume `scrollToLine`
  - [ ] 11.1 Add `scrollToLine?: number | null` prop to `DocumentView` in `apps/desktop/src/renderer/src/components/DocumentView.tsx` (or wherever `DocumentView` is defined)
